...
router.patch("/:id", requireAuth, async (req: Request, res: Response) => {
  //not clean code ..update later

  let { id } = req.params;
  const updatedCaption = req.body.caption;
  const updatedFileName = req.body.url;

  const item = await FeedItem.findByPk(id);
  if (!updatedCaption || !updatedFileName) {
    return res.status(400).send({ message: "File url/ caption is required" });
  }
